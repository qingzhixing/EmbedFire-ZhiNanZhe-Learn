...
int main(void)
{
	...
	*/
	// 清空设置
	GET_ADDR_DATA_BYTE(GPIOB_CRL_BASE) &= ~(0xF << (4 * OPEN_GPIOB_IDX));
	// 设置控制置为0b0001
	SET_DIGIT_1(GET_ADDR_DATA_BYTE(GPIOB_CRL_BASE), 4 * OPEN_GPIOB_IDX);

	// LED_R = GPIOB_(OPEN_GPIOB_IDX)输出 0 开灯
	SET_DIGIT_0(GET_ADDR_DATA_BYTE(GPIOB_ODR_BASE), OPEN_GPIOB_IDX);
	while (1)
		;
}
